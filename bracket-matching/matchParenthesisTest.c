...
void test_1matching_brackets_gives_1(){
        char expr[]="[i should pass as i am valid]";
        ASSERT(doBracketMatch(expr));
};
void test_2gives_0_as_brackets_not_matched(){
        char expr[] = "i{am(not)correct}expression[";
        ASSERT(0 == doBracketMatch(expr));
};
void test_3expression_with_valid_bracket_sequence_gives_1(){
        char expr[] = "{node(server[.{js}suraj]@)thoughtworks}";
        ASSERT(doBracketMatch(expr));
};
void test_4expression_with_valid_bracket_sequence_gives_1(){
